...
import 'package:coka/api/model/source_response.dart';
import 'package:coka/api/model/tag_response.dart';
...
import 'package:coka/screen/main/widget/multi_select_work_space_bottomsheet.dart';
...
import 'package:coka/screen/workspace/components/multi_select_stage_select_bottomsheet.dart';
import 'package:coka/screen/workspace/main_controller.dart';
import 'package:coka/screen/workspace/pages/add_customer.dart';
import 'package:coka/screen/workspace/pages/add_team.dart';
import 'package:flutter/material.dart';
import 'package:gap/gap.dart';
import 'package:get/get.dart';

void showCustomerFilterHomeBottomSheet() {
  CustomerHomeController wmController = Get.put(CustomerHomeController());
...
class _CustomerFilterBottomSheetState extends State<CustomerFilterBottomSheet> {
  ...
  void initState() {
    ...
    Future.wait([
      CustomerApi()
          .getTagListV2(homeController.workGroupCardDataValue["id"])
          .then((value) {
        TagsGetpagingResponse tagsResponse =
            TagsGetpagingResponse.fromJson(value);

        setState(() {
          bonusTags = tagsResponse.content
                  ?.map((e) => ChipData(e.id ?? '', e.name ?? ''))
                  .toList() ??
              [];
        });
      }),
      CustomerApi()
          .getSourceListV2(homeController.workGroupCardDataValue["id"])
          .then((value) {
        setState(() {
          SourceGetpagingResponse sourceResponse =
              SourceGetpagingResponse.fromJson(value);
          bonusSources = sourceResponse.content
                  ?.map((e) => ChipData(e.id ?? '', e.name ?? ''))
                  .toList() ??
              [];
        });
  ...
  Widget build(BuildContext context) {
    return GetBuilder<CustomerHomeController>(builder: (controller) {
      return Container(
                          ...
                          Column(
                            ...
                            children: [
                              buildFilterItem(
                                  ...
                                  title: controller
                                          .workSpaceFilterList.value.isEmpty
                                      ? "Chọn không gian làm việc"
                                      : controller.workSpaceFilterList.value
                                          .map((e) => e["name"])
                                          .join(", "),
                                  onPressed: () {
                                    showModalBottomSheet(
                                      ...
                                      builder: (context) =>
                                          MultiWorkSpaceSelectBottomSheet(
                                        selectedList: controller
                                            .workSpaceFilterList.value
                                            .cast<Map<String, dynamic>>(),
                                        dataFilterList: homeController
                                            .workGroupCardDataList
                                            .toList()
                                            .cast<Map<String, dynamic>>(),
                                        onSelected: (data) {
                                          controller.workSpaceFilterList.value =
  ...
  Padding buildTagFilterItem(CustomerHomeController controller) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Padding buildFilterItem({
    ...
    final wmController = Get.put(CustomerHomeController());
    return Padding(
                              ...
                              name == "Đối tượng phụ trách"
                                  ...
                                  : name == "Trạng thái"
                                      ...
                                      : wmController.ratingFilterList
                                          .removeWhere((e2) =>
                                              e2["value"] == e1["value"]);
                              if (name == "Không gian làm việc") {
                                wmController.workSpaceFilterList
                                    .removeWhere((e2) => e2["id"] == e1["id"]);
                              }
                              wmController.update();
                            },
                          );
