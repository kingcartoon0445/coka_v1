...
import '../screen/workspace/pages/add_team.dart';
...
class ChipsInputState<T> extends State<ChipsInput<T>>
  ...
  void selectSuggestion(T data) {
    if (!_hasReachedMaxChips) {
      ...
      setState(() => _suggestions?.remove(data));
  ...
  void deleteChip(T data) {
    if (widget.enabled) {
      ...
      if (sourceMenu.contains(data) || categoryMenu.contains(data)) {
        setState(() => _suggestions?.add(data));
      }
      _suggestionsStreamController.add(_suggestions);
  ...
  void _onSearchChanged(String value) async {
    if (value.contains(",") && widget.allowChipEditing) {
      final valueData = value.replaceFirst(",", "");
      selectSuggestion(ChipData(valueData, valueData) as T);
      return;
    }
    ...
    if (_searchId == localId && mounted) {
      setState(() => _suggestions =
          results.where((r) => !_chips.contains(r)).toList(growable: true));
      if (value.isNotEmpty) {
        _suggestions?.insert(0, ChipData(value, value) as T);
      }
    }
    _suggestionsStreamController.add(_suggestions ?? []);
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    print({
      "value": value.replacementCharactersCount,
      "oldTextEditingValue": oldTextEditingValue.replacementCharactersCount
    });
