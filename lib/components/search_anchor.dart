...
///
/// The `controller` callback provided to [CustomSearchAnchor.builder] can be used
...
///
/// The `controller` callback provided to [CustomSearchAnchor.suggestionsBuilder] can be used
...
///
/// The search view's route can either be shown by creating a [CustomSearchController]
/// and then calling [CustomSearchController.openView] or by tapping on an anchor.
/// When the anchor is tapped or [CustomSearchController.openView] is called, the search view either
/// grows to a specific size, or grows to fill the entire screen. By default,
/// the search view only shows full screen on mobile platforms. Use [CustomSearchAnchor.isFullScreen]
...
/// to explicitly call [CustomSearchController.openView].
///
...
///
/// {@tool dartpad}
/// This example shows how to use an IconButton to open a search view in a [CustomSearchAnchor].
/// It also shows how to use [CustomSearchController] to open or close the search view route.
///
...
/// [CustomSearchAnchor] for a title.
///
...
class CustomSearchAnchor extends StatefulWidget {
  /// Creates a const [CustomSearchAnchor].
  ///
  ...
  /// Create a [CustomSearchAnchor] that has a [SearchBar] which opens a search view.
  ///
  ...
  /// {@end-tool}
  factory CustomSearchAnchor.bar(
          {Widget? barLeading,
...
class _ViewContentState extends State<_ViewContent> {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget viewDivider = DividerTheme(
      ...
      child: const Divider(height: 1, thickness: 0),
    );
...
class _SearchAnchorWithSearchBar extends CustomSearchAnchor {
  _SearchAnchorWithSearchBar(
      {Widget? barLeading,
      Iterable<Widget>? barTrailing,
      String? barHintText,
...
/// A controller to manage a search view created by [CustomSearchAnchor].
///
/// A [CustomSearchController] is used to control a menu after it has been created,
/// with methods such as [openView] and [closeView]. It can also control the text in the
/// input field.
///
/// See also:
///
/// * [CustomSearchAnchor], a widget that defines a region that opens a search view.
...
/// usually created by a [CustomSearchAnchor.builder]. The builder provides a
/// [CustomSearchController] that's used by the search bar's [SearchBar.onTap] or
...
/// [CustomSearchAnchor.builder] property. The [SearchBar] also includes a leading search
...
/// See also:
///
/// * [CustomSearchAnchor], a widget that typically uses an [IconButton] or a [SearchBar]
